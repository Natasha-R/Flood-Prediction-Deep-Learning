...
def find_sentinel2_availability(data_folder):
    """
    ...
    availability.to_csv(f"{data_folder}/metadata/sentinel2_availability.csv", index=False) 

def find_minimal_cloud_cover(data_folder):
    """
    ...
    aois = gpd.read_file(f"{data_folder}/metadata/aoi_extent.geojson")
    availability = pd.read_csv(f"{data_folder}/metadata/sentinel2_availability.csv")
    ...
    aois.to_file(f"{data_folder}/metadata/aoi_availability.geojson")

def download_sentinel2(data_folder):
    """
    ...
    aois = gpd.read_file(f"{data_folder}/metadata/aoi_availability.geojson")
...
def create_sentinel2_rasters(data_folder):
    """
    ...
    aoi_avail = gpd.read_file(f"{data_folder}/metadata/aoi_availability.geojson")
    ...
    for index in tqdm(range(len(raster_extents))):
        ...
        if os.path.isfile(f"{sentinel2_raster_folder}/{subevent}.tif"):
            continue

        if any([not os.path.isdir(f"{data_folder}/full_subevent/sentinel_2/aoi_{aoi_id}") for aoi_id in aoi_ids]):
            print("Data is missing for subevent:", subevent, flush=True)
            continue
        ...
        for aoi_id in aoi_ids:
            ...
            aoi_groups = os.listdir(f"{data_folder}/full_subevent/sentinel_2/aoi_{aoi_id}")
            aoi_groups.sort()
            ...
            for aoi_group in aoi_groups:
                ...
                sub_aoi_paths = [os.path.join(root, file) for root, dirs, files in os.walk(f"{data_folder}/full_subevent/sentinel_2/aoi_{aoi_id}/{aoi_group}") for file in files if file.endswith(".tiff")]
                gdal.PushErrorHandler('CPLQuietErrorHandler')
                gdal.Warp(f"{sentinel2_raster_folder}/{subevent}_{aoi_id}_{aoi_group}.tif", sub_aoi_paths, 
                ...
                with rasterio.open(f"{sentinel2_raster_folder}/{subevent}_{aoi_id}_{aoi_group}.tif") as sentinel2_file:
                ...
                with rasterio.open(f"{sentinel2_raster_folder}/{subevent}_{aoi_id}_{aoi_group}.tif", 'w', **meta) as file:
                    for band_index in range(count):
                        file.write(sentinel2_raster[band_index], band_index + 1)
                        file.write(date_difference, count + 1)

                all_aoi_paths.append(f"{sentinel2_raster_folder}/{subevent}_{aoi_id}_{aoi_group}.tif")

        # join all of the AOI rasters together to form the overall subevent raster
        gdal.PushErrorHandler('CPLQuietErrorHandler')
        gdal.Warp(f"{sentinel2_raster_folder}/{subevent}.tif", all_aoi_paths, 
        ...
        with rasterio.open(f"{sentinel2_raster_folder}/{subevent}.tif") as sentinel2_file:
            sentinel2_raster = sentinel2_file.read()
            meta = sentinel2_file.meta.copy()
            count = sentinel2_file.count
        ...
        size_bytes = os.path.getsize(f"{sentinel2_raster_folder}/{subevent}.tif")
        ...
        with rasterio.open(f"{sentinel2_raster_folder}/{subevent}.tif", 'w', **meta, compress="LZW") as file:
            for band_index in range(count):
                file.write(sentinel2_raster[band_index], band_index + 1)
        ...
        for path in all_aoi_paths:
           os.remove(path)
        os.remove(f"{sentinel2_raster_folder}/{subevent}.tif.aux.xml")
...
if __name__ == "__main__":
    ...
    if args.find_sentinel2_availability:
        find_sentinel2_availability(args.data_folder)

    if args.find_minimal_cloud_cover:
        find_minimal_cloud_cover(args.data_folder)

    if args.download_sentinel2:
        download_sentinel2(args.data_folder)
    
    if args.create_sentinel2_rasters:
        create_sentinel2_rasters(args.data_folder)