...
class FloodDataset(torch.utils.data.Dataset):
     ...
     def __getitem__(self, index):
          ...
          data = {"local_features": local_features, "local_label": local_label}

          data = self.transform(data)

          return data

def create_data_loaders(config, data_folder, subset=None, subevent=None, event=None):

     if subset or subevent or event: 
...
class Normalize(object):
     ...
     def __call__(self, data):
          return data
          ## need to normalize separately based on features, and also on what features are included
