...
def find_subevent_descriptions(data_folder, global_folder):
    ...
    lvl6_basin = gpd.read_file(f"{global_folder}/global_basins/lev06_basin.geojson").to_crs("EPSG:6933")
    ...
    characteristics = ["event", "subevent", "mean_dem", "std_dem", "mean_precipitation_daily", "max_precipitation_daily", "std_precipitation_daily", "total_precipitation_daily",
                       ...
                       "mean_root_soil_moisture_one_week", "mode_soil_class", "month", "year", "lvl4_basin_area", "lvl5_basin_area", "lvl6_basin_area", "aoi_area", 
                       "proportion_urban", "continent", "country", "latitude", "longitude", "flood_cause", "number_of_patches"]
    ...
    for index in tqdm(range(len(raster_extents))):
        ...
        num_flood = num_extent
        num_non_flood = num_trace + num_aoi
        num_total = num_flood+num_non_flood
        ...
        lulc = tf.imread(f"{data_folder}/full_subevent/raster_land_cover/{subevent}.tif")
        ...
        for basin, basin_name in zip([lvl4_basin, lvl5_basin, lvl6_basin], ["lvl4_basin_area", "lvl5_basin_area", "lvl6_basin_area"]):
        ...
        pd.DataFrame(subevent_descriptions).to_csv(f"{data_folder}/metadata/subevent_descriptions_newest.csv", index=False)
